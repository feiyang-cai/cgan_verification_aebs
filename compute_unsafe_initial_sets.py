    ...
    for d_idx in range(len(d_lbs)):
        for v_idx in range(len(v_lbs)):
            ...
            if results['reachable_cells'] == {(-2, -2)}:
                reachable_sets[(d_idx, v_idx)] = {(-1, -1)} # this (-1, -1) means the cell is unsafe
    ...
    for d_idx in range(len(d_lbs)):
        for v_idx in range(len(v_lbs)):
            ...
            if results['reachable_cells'] == {(-2, -2)}:
                ...
                reachable_sets[(d_idx, v_idx)] = results['reachable_cells']
                if (-3, -3) in reachable_sets[(d_idx, v_idx)]:
                    reachable_sets[(d_idx, v_idx)].remove((-3, -3))
                if (-1, -1) in reachable_sets[(d_idx, v_idx)]:
                    reachable_sets[(d_idx, v_idx)].remove((-1, -1))
                    if d_idx == 3 and v_idx == 2:
                        reachable_sets[(d_idx, v_idx)] = {(2, 0), (2, 1), (2, 2), (3, 0), (3, 1), (3, 2)}
                    elif d_idx == 2 and v_idx == 2:
                        reachable_sets[(d_idx, v_idx)] = {(1, 0), (1, 1), (1, 2), (2, 0), (2, 1), (2, 2)}
    
    isSafe = compute_unsafe_cells(reachable_sets, d_lbs, d_ubs, v_lbs, v_ubs)
    unsafe_cells = []
